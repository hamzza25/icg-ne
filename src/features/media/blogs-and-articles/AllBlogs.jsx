...
function AllBlogs() {
  ...
  return (
    ...
    <div className="container mx-auto px-6 lg:px-16 py-10  dark:bg-gray-800 ">
      ...
      {isLoading && (
        <h1 className="text-center mt-10 dark:text-gray-300">Loading...</h1>
      )}

      <div className="grid mt-10 sm:grid-cols-1 gap-x-4 gap-y-5 md:grid-cols-2 2xl:grid-cols-4 2xl:gap-x-6 2xl:gap-y-6">
        {data?.blogs.map((item) => (
          <div
            className="relative h-[400px] dark:border dark:border-gray-600 w-full bg-[#FFFFFF] dark:bg-gray-800 rounded-3xl shadow-lg"
            key={item._id}
          >
            <img
